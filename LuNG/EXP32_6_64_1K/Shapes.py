...
class Shapes:
    ...
    def buildnn(self) :
        self.net = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(32,6),           # Bottleneck layer has 6 neurons
            nn.Tanh(),
            nn.Linear(6,64),
            nn.Tanh(),
            nn.Linear(64,1024),
    ...
    def buildgen(self) :
        self.gen = nn.Sequential(
            nn.Linear(6,64),           # 6 layer bottleneck is input 
            nn.Tanh(),
    ...
    def prepOut(self,rand=0,startimg=27,endimg=36) :
        ...
        if rand==0:      # Generate outputs from X Sh.images (real training) data
            ...
            images=-rand
            X = torch.Tensor(images,6).fill_(0).type(self.dtype)
            ...
            images=rand
            X = torch.rand(images,6).type(self.dtype)*2-1
            predict=self.gen(Variable(X)).view(images,sz,sy,sx).type(torch.FloatTensor)
