...
#pragma omp parallel for
  for (i = 0; i < nbcomp; ++i)
    {
      if (comp_sz[i] < 5) {
        printf ("SJK: Pruning nodule of size %d\n", comp_sz[i]);
	continue;
      } 
      int j, k;
...
/* SJK: open up ranges to get more objects */
      printf ("SJK: d=%.2f (%.2f,%.2f) e=%.2f (%.2f) v=%.2f (%.2f,%.2f)\n", diameter, diameTMin, diameTMax, elongation, elongationTMax, volume, volumTMin, volumTMax);
      if (diameter < 0.6*diameTMin)
	continue;
      if (diameter > 1.5*diameTMax)
	continue;
      if (elongation > 2.0*elongationTMax)
	continue;
      if (volume > volumTMax)
	continue;
      if (volume < 0.23*volumTMin)
	continue;
...
/* SJK : Loosen constraints to allow more objects */
      if (area > 4.0*areaTMax || area < 0.2*areaTMin)
	continue;
      if (roundDegree < 0.2*circulTMin)
      	continue;

      // Good candidate nodule.
      printf ("SJK: Keeping nodule of size %d\n", comp_sz[i]);
      for (j = 0; j < comp_sz[i]; ++j)
