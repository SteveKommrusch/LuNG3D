		       ...
		       image3DReal** __ALNSB_RESTRICT_PTR output)
{
  ...
  *output = image3DReal_alloc (input->slices, input->rows, input->cols);
  ...
  if (fp) {
    printf("INFO: Overriding segmentation step input.  Original size = %d,%d,%d\n",slices,rows,cols);
    if ((rows < 716) || (cols < 716)) {
        printf("ERROR: rows and cols are assumed to be at least 716 for the current override code\n");
    }
    // Override the input using the provided CSV file, which contains
    // lines with example nodules of size 20x40x40
#pragma omp parallel for private(j,k)
    ...
    iter=0; // iter will count the nodules on input
    t = fscanf(fp,"%f,",&voxel);
    while (t != EOF) {  
       // CSV images are always 20x40x40
       for (i = 0; i < 20; i++) {
          for (j = 0; j < 40; j++) {
             for (k = 0; k < 40; k++) {
               ur[i+5+(int)(iter/(15*15))*25][j+7+((int)(iter/15)%15)*47][k+7+(iter%15)*47] = voxel;
               ut[i+5+(int)(iter/(15*15))*25][j+7+((int)(iter/15)%15)*47][k+7+(iter%15)*47] = voxel > 0.1;
               if ((k == 0) && (voxel > 0.1)) {
                 printf("SJKINFO: edge is on: %d,%d,%d,%d = %f\n",iter,i+5+(int)(iter/(15*15))*25,j+7+((int)(iter/15)%15)*47,k+7+(iter%15)*47,voxel);
               }
               if (t == EOF)
                 printf("ERROR! unexpected end of override\n");
               t = fscanf(fp,"%f,",&voxel);
             }
          }
       }
    ...
    printf("INFO: %d nodules loaded from segInputOverride.csv",iter);
    fclose(fp);
    return;
  }
